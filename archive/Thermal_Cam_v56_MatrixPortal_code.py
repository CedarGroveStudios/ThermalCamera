# Thermal_Cam_v56_MatrixPortal.py
# 2021-05-28 v5.6
# MatrixPortal version
...
# Instantiate display and initialize the matrix
matrix = Matrix()
...
mode_button = DigitalInOut(board.A0)
...
def update_image_frame(selfie=False):  # Get camera data and update display
    for row in range(0, GRID_AXIS):
        for col in range(0, GRID_AXIS):
            ...
            color = index_to_rgb(round(color_index * PALETTE_SIZE, 0) / PALETTE_SIZE, 1)
...
# status_label.anchored_position = (22, 26)
...
while True:
    ...
    if not hold_button.value:  # # Toggle display hold (shutter)
        play_tone(1319, 0.030)  # E6
        display_hold = not display_hold

        while not hold_button.value:
            time.sleep(0.25)

    if not mode_button.value:  # Toggle image/histogram mode (display mode)
        play_tone(659, 0.030)  # E5
        if display_image:
            flash_status("IMAGE", 0.25)
        else:
            flash_status("HISTO", 0.25)
        display_image = not display_image

        while not mode_button.value:
            time.sleep(0.25)

    if not focus_button.value:  # toggle focus mode (display focus)
