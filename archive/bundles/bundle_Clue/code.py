...
# Instantiate display and neopixels
display = board.DISPLAY
# Load the text font from the fonts folder
font_0 = bitmap_font.load_font("/fonts/OpenSans-9.bdf")

pixels = neopixel.NeoPixel(board.NEOPIXEL, 5, brightness=0.1, pixel_order=neopixel.GRB)
pixels.brightness = 0.1  # Set NeoPixel brightness
pixels.fill(0)  # Clear all NeoPixels

# Define buttons
focus_button = DigitalInOut(board.BUTTON_B)
focus_button.direction = Direction.INPUT
focus_button.pull = Pull.UP

hold_button = DigitalInOut(board.D0)
hold_button.direction = Direction.INPUT
hold_button.pull = Pull.UP

mode_button = DigitalInOut(board.BUTTON_A)
...
while True:
    ...
    if not mode_button.value:
        ...
        if display_image:
            ...
            alarm_label.color = alarm_value.color = WHITE
            max_label.color = max_value.color = WHITE
            ave_label.color = ave_value.color = WHITE
            min_label.color = min_value.color = WHITE
        else:
            flash_status("-IMAGE-", 0.5)
            min_histo.color = None
            max_histo.color = None
            range_histo.color = None
            alarm_label.color = alarm_value.color = BLACK
            max_label.color = max_value.color = BLACK
            ave_label.color = ave_value.color = BLACK
            min_label.color = min_value.color = BLACK
