# Thermal_Cam_v40_PyGamer_code.py
# 2021-04-25 v4.0
# (c) 2021 Jan Goolsbey for Adafruit Industries
...
def update_image_frame():  # Get camera data and display
    ...
    for row1 in range(0, 8):  # Parse camera data list and update display
        for col1 in range(0, 8):
            ...
            color_index = map_range(value, MIN_RANGE_C, MAX_RANGE_C, 0.0, 1.0)
            image_group[((row1 * 8) + col1) + 1].fill = index_to_rgb(color_index)
    ...
    for col2 in range(0, 8):  # Display histogram
        for row2 in range(0, 8):
            if histo_bucket[col2] / 8 > 7 - row2:
                image_group[((row2 * 8) + col2) + 1].fill = index_to_rgb(col2 / 8)
            else:
...
max_histo = Label(font, text="", color=CYAN, max_glyphs=3)
