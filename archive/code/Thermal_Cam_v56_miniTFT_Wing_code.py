...
def update_histo_frame():  # Calculate and display histogram
    ...
    for row in range(0, GRID_AXIS):  # Collect camera data and calculate histo
        for col in range(0, GRID_AXIS):
            histo_index = int(map_range(grid_data[col, row], 0, 1, 0,
                                        GRID_AXIS - 1))
    ...
    for col in range(0, GRID_AXIS):  # Display histogram
        for row in range(0, GRID_AXIS):
            if histogram[col] / histo_scale > GRID_AXIS - 1 - row:
                image_group[((row * GRID_AXIS) + col)].fill = index_to_rgb(round((col / GRID_AXIS), 3))
            else:
...
for row in range(0, GRID_AXIS):
    for col in range(0, GRID_AXIS):
        ...
        cell = Rect(x=cell_x, y=cell_y, width=CELL_SIZE, height=CELL_SIZE,
...
status_label.anchored_position = ((WIDTH // 2) + (GRID_X_OFFSET // 2),
                                  HEIGHT // 2)
...
range_histo.anchored_position = ((WIDTH // 2) + (GRID_X_OFFSET // 2),
                                 HEIGHT - 10)
...
while True:
    ...
    for row in range(0, 8):  # Constrain sensor values to valid range
        for col in range(0, 8):
            sensor_data[col, row] = min(max(sensor_data[col, row],
                                        0), 80)
