...
# Thermal_Cam_v61_PyGamer_code.py
# 2021-07-07 v6.1
...
sensor_data = numpy.array(range(n * n)).reshape((n, n))  # Color index narray
grid_data = numpy.zeros(((2 * n) - 1, (2 * n) - 1))  # 15x15 color index narray
histogram = numpy.zeros((2 * n) - 1)  # Histogram accumulation narray
...
def update_histo_frame():  # Calculate and display histogram
    ...
    histogram = numpy.zeros(GRID_AXIS)  # Clear histogram accumulation array
    ...
    histo_scale = numpy.max(histogram) / (GRID_AXIS - 1)
    if histo_scale <= 0:
        histo_scale = 1
...
while True:
    ...
    sensor_data = numpy.array(sensor)  # Copy to narray

    t3 = time.monotonic()  # Time marker: Constrain Sensor Values
    # numpy.clip(sensor_data, 0, 80)
    ...
    v_max = numpy.max(sensor_data)
    v_min = numpy.min(sensor_data)
    v_ave = numpy.mean(sensor_data)
